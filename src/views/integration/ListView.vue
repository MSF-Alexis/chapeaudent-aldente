...
const highlightedExample = computed(() => {
  ...
  return prettifyCode(currentFiche.value.exemple, { type: 'html' })
})
...
<template>
  <BaseModal v-model="isOpen" title-id="integration-modal-title">
    <template #title>
      <div class="fiche-detail__header">
        <h2 id="html-modal-title" class="modal__title fiche-detail__title">
          {{ currentFiche?.competence || 'Détail de la fiche Intégration' }}
        </h2>
  ...
  <FicheListView title="Notions d’intégration web"
    ...
    :fiches="dataSheetStore.integration" @update:modal="handleOpenedModal" />
