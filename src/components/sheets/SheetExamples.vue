...
import type { Example } from '@/types/Sheet';
...
<template>
  <section v-if="examples?.length" class="sheet-section sheet-section--examples">
    ...
    <div v-if="examples.length > 1" class="example-tabs">
      <button v-for="(example, index) in props.examples" :key="example.id" @click="selectExample(index)"
    ...
    <div v-if="examples[activeExampleIndex]" class="example-content">
      ...
      <h3 v-if="examples.length === 1 && examples[activeExampleIndex]" class="example-title">
